...
def make_env(scenario_name, arglist, benchmark=False):
    ...
    # create world
    world = scenario.make_world(num_obs=3)
...
def train(arglist):
    with U.single_threaded_session():
        ...
        rew_file_name = arglist.plots_dir + arglist.exp_name + '_rewards_final.pkl'
        agrew_file_name = arglist.plots_dir + arglist.exp_name + '_agrewards_final.pkl'
        loss_file_name = arglist.plots_dir + arglist.exp_name + '_losses.pkl'
        ...
        with open(rew_file_name, 'wb') as f_rew, open(agrew_file_name, 'wb') as f_agrew, open(loss_file_name, 'wb') as f_loss:
            ...
            while True:
                ...
                if terminal and (len(episode_rewards) % arglist.save_rate == 0):
                    ...
                    pickle.dump(
                        [np.mean(episode_rewards[-arglist.save_rate:])], f_rew)
                    for rew in agent_rewards:
                        final_ep_ag_rewards.append(
                            np.mean(rew[-arglist.save_rate:]))
                        pickle.dump(np.mean(rew[-arglist.save_rate:]), f_agrew)
